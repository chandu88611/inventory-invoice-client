...
const DocCreateEditForm = () => {
	...
	const { data: customers } = useState();
	const { data: singleDoc } = useState();
	const [createDoc, { isLoading, isSuccess }] = useState();
	...
	const getAllCustomers =async () =>{
		console.log("chadb")
		try {
			const res= await axios.get("http://localhost:5000/api/customer/get-all-customers",{
								headers: {
					...
					console.log(res)
					setCustomer1(res.data.customers);		  
		} catch (error) {
			console.log(error);
		}
	}
    ...
    onChange={(e) => {
      ...
      setItems((currentItem) =>
        produce(currentItem, (v) => {
		  ...
		  const selectedProduct = products.find((product) => product.name === itemName);
		  if(selectedProduct){
